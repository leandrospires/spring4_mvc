...
public class ContaDAO {
	...
	public void adiciona(Conta conta) {
		String sql = "insert into contas (descricao, fabricante, paga, valor, tipo) values (?,?,?,?,?)";
		PreparedStatement stmt;
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, conta.getDescricao());
			stmt.setString(2, conta.getFabricante());
			stmt.setBoolean(3, conta.isPaga());
			stmt.setDouble(4, conta.getValor());
			stmt.setString(5, conta.getTipo().name());
			//stmt.setString(6, conta.getDataPagamento().toString());
			stmt.execute();
		} catch (SQLException e) {
			throw new RuntimeException(e);
		}
	...
	public void altera(Conta conta) {
		//String sql = "update contas set descricao = ?, fabricante = ?, paga = ?, dataPagamento = ?, tipo = ?, valor = ? where id = ?";
		String sql = "update contas set descricao = ?, fabricante = ?, valor = ?, tipo = ?, paga = ? where id = ?";
		PreparedStatement stmt;
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, conta.getDescricao());
			...
			//stmt.setDate(6, conta.getDataPagamento() != null ? new Date(conta.getDataPagamento().getTimeInMillis()) : null);
	...
	private Conta populaConta(ResultSet rs) throws SQLException {
		...
		conta.setFabricante(rs.getString("fabricante"));
